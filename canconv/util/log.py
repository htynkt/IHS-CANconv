...
def save_mat_data(sr, scale, output_dir):
    # 定义保存矩阵数据的目录路径，位于输出目录下的 "results" 文件夹
    mat_dir = os.path.join(output_dir, "results")
    # 如果该目录不存在，则创建它
    if not os.path.exists(mat_dir):
        os.makedirs(mat_dir)

    # 获取输入数据 sr 的第一个维度大小（通常是批次大小）
    data_size = sr.shape[0]
    # 遍历每个样本
    for i in range(data_size):
        # 将当前样本的维度从 [c, h, w]（通道在前）重新排列为 [h, w, c]（通道在后）
        # 并乘以 scale 缩放比例，将其转换为原始尺度
        batch = rearrange(sr[i], 'c h w -> h w c') * scale
        # 将张量移动到 CPU 并转换为 NumPy 数组
        batch = batch.cpu().numpy()

        # 构造输出文件的路径，文件名为 "output_mulExm_{i}.mat"
        out_file = os.path.join(mat_dir, f'output_mulExm_{i}.mat')
        # 使用 scipy.io.savemat 将数据保存为 .mat 文件
        # 数据以字典形式保存，键为 "sr"，值为当前样本的 NumPy 数组
